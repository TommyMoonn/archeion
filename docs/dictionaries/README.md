...
The builder reads each package's StarDict metadata to retain its concise upstream dictionary name as attribution and normalize its stated redistribution licence. Package descriptions remain a maintenance input for licence inspection but are not copied into the user-facing catalog. `scripts/dictionaries/freedict_exclusions.json` contains only current releases rejected by Archeion's existing StarDict validator, with the exact compatibility reason and upstream version, URL, byte size, and SHA-512. An exclusion applies only while that complete release identity matches current metadata. Any changed release, unexpected candidate-construction failure, or transient download failure stops the build instead of silently reducing catalog coverage.
