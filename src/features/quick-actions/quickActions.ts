...
export type QuickActionCommand = {
  ...
  group: "Archive" | "Library" | "Navigate" | "Reader" | "System";
  id: string;
...
export class QuickActionsRegistry {
  ...
  register(sourceId: string, commands: readonly QuickActionCommand[]): () => void {
    ...
    return () => {
      if (this.registrations.get(sourceId)?.token !== token) {
        return;
      }
  ...
  private publish(): void {
    ...
    for (const command of commands) {
      if (commandIds.has(command.id)) {
        throw new Error(`Duplicate Quick Actions command id: ${command.id}`);
      }
...
export function createQuickActionIndex(
  ...
  return commands.map((command) => ({
    command,
    disabledReason: createSearchTextVariants(command.disabledReason),
...
export function searchQuickActions(
  ...
  return index
    .map((entry) => {
      ...
      if (!searchFieldsMatchQuery(fields, query)) {
        return null;
      }
...
export function isTextEntryTarget(target: EventTarget | null): boolean {
  ...
  while (element) {
    ...
    if (contentEditable !== null && TEXT_CONTENTEDITABLE_VALUES.has(contentEditable)) {
      return true;
    }

    if (normalizedAttribute(element, "role") === "textbox") {
      return true;
    }
...
export function isQuickActionsShortcut(event: KeyboardEvent): boolean {
  return (
