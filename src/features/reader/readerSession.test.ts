...
describe("reader session keys", () => {
  it("keys reader route sessions by book and start mode only", () => {
    const normalKey = createReaderSessionKey("book-1", "resume");
    const progressOnlyUpdateKey = createReaderSessionKey("book-1", "resume");

    expect(progressOnlyUpdateKey).toBe(normalKey);
    expect(createReaderSessionKey("book-1", "beginning")).not.toBe(normalKey);
